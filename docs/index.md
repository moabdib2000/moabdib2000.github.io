# Miguel Medina
### :material-account-tie: Perfil profesional · Ciberseguridad & Gestión IT

Hola, soy **Miguel Ángel Medina**, profesional con experiencia en **gestión comercial**, **diseño gráfico** y en proceso de especialización en **ciberseguridad**.

---

!!! tip ":material-account-circle-outline: Sobre mí"
    Perfil híbrido con experiencia en **gestión de cuentas**, **control presupuestario** y **optimización de procesos**, con base técnica en **diseño gráfico** (InDesign, Photoshop, Illustrator).  
    Formación reciente en **ciberseguridad** mediante bootcamp especializado. Profesional proactivo, analítico y orientado a resultados.

---

!!! tip "FORMACIÓN"

!!! tip ":material-shield-lock: Ciberseguridad"

    - :material-bug-check: **Pentesting** – Pruebas de penetración
    - :material-security: **Hacking ético** – Evaluación de seguridad
    - :material-monitor-eye: **SIEM** – Monitorización de eventos
    - :material-radar: **Threat Intelligence** – Análisis de amenazas
    - :material-alert-octagon: **Respuesta a incidentes**
    - :material-magnify: **Análisis forense**
    - :material-file-document-check: **Normativa y compliance** – RGPD, LOPD

---

!!! tip ":material-briefcase: Gestión y Administración"

      - :material-calculator: **Control presupuestario**
      - :material-cart: **Compras y aprovisionamiento**
      - :material-account-group: **Gestión de cartera de clientes**
      - :material-database: **Administración con CRM / ERP**

---

!!! tip ":material-laptop: Tecnología y Sistemas"

      - :material-microsoft-windows: **Windows**
      - :material-apple: **macOS**
      - :material-linux: **Linux**
      - :material-palette: **Adobe Creative Suite**

---

!!! tip ":material-account-multiple: Habilidades transversales"

      - Coordinación con equipos técnicos
      - Trato con clientes corporativos
      - Optimización de procesos
      - Alta capacidad de aprendizaje

---

!!! tip ":material-rocket-launch: Camino profesional y próximos proyectos"
    La formación en ciberseguridad consolidó mi transición hacia el ámbito IT.  
    Actualmente desarrollo proyectos orientados a:

    - :material-bug: Análisis de vulnerabilidades
    - :material-flask: Creación de laboratorios de prácticas
    - :material-robot: Automatización de tareas
    - :material-code-braces: Desarrollo de scripts

    Este sitio documenta mi evolución y aprendizaje continuo.

---

!!! tip ":material-email-outline: Contacto"

      - :material-email: **Email** · [info@miguelmedina.es](mailto:info@miguelmedina.es)  
      - :material-linkedin: **LinkedIn** · [Perfil profesional](https://linkedin.com/in/miguel-angel-medina-lozano)  
      - :material-web: **Portfolio** · En construcción  

[:material-send: Enviar mensaje](mailto:info@miguelmedina.es){ .md-button .md-button--primary }

---

Gracias por visitar este espacio. Siempre abierto a intercambiar ideas y colaborar en proyectos.
